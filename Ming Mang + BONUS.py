...
def alreadySeen(board,saveList):    #Fonction qui vérifie que le plateau de jeu actuel ne se situe pas déjà dans la liste des plateaux vus
    if board in saveList:
        return True
    else:   #Si la configuration actuelle n'a pas déjà été vue, on sauvegarde l'actuel plateau dans la liste
        boardCopy = copy.deepcopy(board)    #Copie du plateau de jeu
        saveList.append(boardCopy)  #Sauvegarde du plateau actuel
        return False

def mingMang(n):
    board = newBoard(n) #On appelle la fonction newBoard qui va créer un nouveau plateau de jeu
    boardCopy = copy.deepcopy(board)    #Copie du plateau de jeu au début
    saveList = []   #On définit une liste vide
    saveList.append(boardCopy)  #Sauvegarde du premier plateau de jeu
    ...
    while lose(board,n,player) == False:    #Tant que la fonction lose retourne False, la boucle fonctionne
        ...
        if alreadySeen(board,saveList) == True: #Retourne au début du tour du joueur si la configuration a déjà été vue
            board = boardCopy   #Le plateau est réinitialisé à son état avant le déplacement
            print("Cette configuration a déjà été vue ! Rejouez !")
            continue
